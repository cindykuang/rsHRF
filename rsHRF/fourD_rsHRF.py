...
def demo_rsHRF(input_file, mask_file, output_dir, para, p_jobs, file_type=".nii", mode="bids", wiener=False, temporal_mask=[]):
   ...
   # for time-series input
        ...
        name = input_file.split('/')[-1].split('.')[0]
        data1 = (np.loadtxt(input_file, delimiter=" ")) #changed!! for my .csv files in particular
        if data1.ndim == 1:
    ...
    bold_sig_deconv = bold_sig.copy()
    bold_sig_deconv = processing. \
                      rest_filter. \
                      rest_IdealFilter(bold_sig_deconv, para['TR'], para['passband_deconvolve'])   
    bold_sig = processing. \
               rest_filter. \
